(ns coinbase.main
  "Kotodama WASM entrypoint for the Coinbase clean-room actor (L4) — Clojure port.
  ...
  facts (`coinbase.<Entity>/<field>`); `*store*` is the in-memory materialization
  used by the contract test and by the WASM runtime before a live engine binds."
  (:require [clojure.string :as str]))

(def ns-prefix "coinbase")
(def tier "L4")
...
(def entity-specs
  [{:entity "Order"   :plural "orders"   :id-prefix "coinbase_ord"
    :fields [:orderId :productId :side :status :clientOrderId :orderType] :required [:side :status]
    :coerce {}                       :refs {:productId "Product"}}
   {:entity "Fill"      :plural "fills"      :id-prefix "coinbase_fil"
    :fields [:entryId :tradeId :orderId :price :size :productId :side] :required [:price :size]
    :coerce {}                       :refs {:orderId "Order" :productId "Product"}}
   {:entity "Account"   :plural "accounts"   :id-prefix "coinbase_acc"
    :fields [:uuid :name :currency :default :active :type]              :required [:uuid :name]
    :coerce {:default :bool :active :bool} :refs {}}
   {:entity "Product"   :plural "products"   :id-prefix "coinbase_pro"
    :fields [:productId :price :volume24h :productType :status]         :required [:price :volume24h]
    :coerce {}                       :refs {:productId "Product"}}
   {:entity "Portfolio" :plural "portfolios" :id-prefix "coinbase_por"
    :fields [:uuid :name :type]                                         :required [:uuid :name]
    :coerce {}                       :refs {}}
   {:entity "PortfolioPosition" :plural "portfoliopositions" :id-prefix "coinbase_pos"
    :fields [:asset :totalBalanceFiat :totalBalanceCrypto :allocation]  :required [:asset :totalBalanceFiat]
    :coerce {:totalBalanceFiat :float :totalBalanceCrypto :float :allocation :float} :refs {}}])
...
(defn emit-facts
  "EAVT facts for one record: {\"coinbase.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "coinbase-compat" :tier tier :entities entities} 200])
...
(defn start! [] :coinbase-compat/ready)
